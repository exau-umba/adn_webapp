...
export function ContratsManagementScreen() {
  ...
  const bundles = useMemo(() => getAllContratBundles(), [key]);
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-1 font-myriad text-sm text-slate-500 dark:text-slate-400">
            Fiches engagement client et agent générées à partir des missions (dossiers liés).
          </p>
        </div>
        <AppButton variant="secondary" size="lg" onClick={() => navigate(ROUTES.missionRegistration)}>
          Nouvelle mission → contrats
        </AppButton>
      </div>
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm dark:bg-slate-900/80">
        ...
        <AppInput className="mt-3" placeholder="Référence mission, dossier CTR-..." />
      </div>
